/*
 * C# port of the minilzo (LZO1X-1) real-time data compression library.
 * Port by Frank Razenberg (https://github.com/zzattack/MiniLZO).
 * Original LZO library (C) Markus F.X.J. Oberhumer, http://www.oberhumer.com/opensource/lzo/
 * LZO is distributed under the terms of the GNU General Public License v2 (or later).
 *
 * Only cosmetic changes were made: namespace/type names adjusted to fit this project.
 * The S4 League resource format stores its payloads as LZO1X streams, so a byte-compatible
 * LZO1X-1 compressor/decompressor is required.
 */

namespace S4League.Resource.Internal;
...
internal static class MiniLzo
{
    ...
    private static int lzo_bitops_ctz32(uint v)
        => MultiplyDeBruijnBitPosition[unchecked((v & (0u - v)) * 0x077CB531U) >> 27];
    ...
    private static unsafe int lzo1x_decompress(byte* @in, uint in_len, byte* @out, ref uint out_len, void* wrkmem)
    {
        byte* op;
        byte* ip;
        uint t;
    ...
    public static unsafe byte[] Decompress(byte[] input, int outputSize)
    {
        byte[] @out = new byte[outputSize];
        uint out_len = 0;
        fixed (byte* pIn = input, wrkmem = new byte[IntPtr.Size * 16384], pOut = @out)
        {
            lzo1x_decompress(pIn, (uint)input.Length, pOut, ref out_len, wrkmem);
        }
        return @out;
    }
}
