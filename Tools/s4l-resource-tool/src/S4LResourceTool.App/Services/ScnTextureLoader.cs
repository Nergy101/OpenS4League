...
/// <summary>A decoded 2D texture (raw BGRA, bottom-up row 0 = top) ready for UV sampling.</summary>
...
/// <summary>Resolves .scn-referenced texture files from the resource archive and decodes them.</summary>
public static class ScnTextureLoader
{
    /// <summary>
    /// Finds the archive entry whose file name equals <paramref name="textureName"/>
    /// (case-insensitive, ignoring any directory) and decodes it. Returns null if not found
    /// or not decodable.
    /// </summary>
    public static ScnTexture? TryLoad(ResourceService resources, string textureName)
    {
        if (resources.Zip is null) return null;
        // The referenced name may include a directory (e.g. "map/foo.tga"); compare the
        // basename against the archive entry's name so we still match.
        var baseName = textureName.Replace('\\', '/');
        var slash = baseName.LastIndexOf('/');
        if (slash >= 0) baseName = baseName[(slash + 1)..];

        var entry = resources.Zip.Values.FirstOrDefault(e =>
    ...
    public static ScnTexture? Decode(byte[] data, string name)
    {
        try
        {
            ...
            // S4 .dds files are stored bottom-up; flip them vertically so UV mapping matches
            // the (top-down) TGA convention. This mirrors the reference tool's DDS row-swap.
    ...
    /// <summary>Flips the BGRA buffer vertically (top row <-> bottom row).</summary>
