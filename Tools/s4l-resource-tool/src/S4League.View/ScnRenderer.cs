...
public static class ScnRenderer
{
    ...
    private static readonly byte[] Bg = { 24, 26, 32, 255 };
        ...
        string? textureOverride = null)
    {
        ...
        var dir = new Vector3(cp * syaw, sp, cp * cyaw);
                ...
                for (int y = y0; y <= y1; y++)
                {
                    for (int x = x0; x <= x1; x++)
                    {
                        ...
                        if (tex is not null)
                        {
                            ...
                            ScnTextureLoader.Sample(tex, u, v, out byte tr, out byte tg, out byte tb, out byte ta);
