...
namespace OpenS4L.Server.Chat
{
    public class FriendManager : ISaveable, IReadOnlyCollection<Friend>
    {
        ...
        public FriendManager(IdGeneratorService idGeneratorService, DatabaseService databaseService, ChatMapper mapper)
        {
        ...
        public async Task Save(GameContext db)
        {
            if (!_friendsToRemove.IsEmpty)
            {
                ...
                await db.PlayerFriends.Where(x => idsToRemove.Contains(x.Id)).ExecuteDeleteAsync();
            }
