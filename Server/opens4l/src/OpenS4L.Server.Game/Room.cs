...
namespace OpenS4L.Server.Game
{
    public class Room
    {
        ...
        protected virtual void OnPlayerJoining(Player plr)
        {
            ...
            RoomManager.Channel.Broadcast(new RoomChangeRoomInfoAck2Message(_mapper.ToRoom2Dto(this)));
        ...
        protected virtual void OnOptionsChanged()
        {
            ...
            Broadcast(new RoomChangeRuleAckMessage(_mapper.ToChangeRuleDto(Options)));
            RoomManager.Channel.Broadcast(new RoomChangeRoomInfoAck2Message(_mapper.ToRoom2Dto(this)));
        }

        public Room(ILogger<Room> logger, GameRuleResolver gameRuleResolver, GameDataService gameDataService,
            ISchedulerService schedulerService, IMessageBus messageBus, GameMapper mapper)
        {
            _logger = logger;
        ...
        public RoomJoinError Join(Player plr)
        {
            ...
            Broadcast(new RoomEnterPlayerInfoAckMessage(_mapper.ToRoomPlayerDto(plr)));
            plr.Session.Send(new RoomEnterRoomInfoAck2Message(_mapper.ToEnterRoomInfo2Dto(this)));
            ...
            plr.Session.Send(new RoomPlayerInfoListForEnterPlayerAckMessage(
                _players.Values.Select(x => _mapper.ToRoomPlayerDto(x)).ToArray())
            );
            OnPlayerJoining(plr);
        ...
        public RoomChangeRulesError ChangeRules(ChangeRule2Dto options)
        {
            ...
            Broadcast(new RoomChangeRuleNotifyAck2Message(_mapper.ToChangeRule2Dto(Options)));
