...
namespace OpenS4L.Server.Game
{
    public class ClanManager : IHostedService, IReadOnlyCollection<Clan>
    {
        ...
        private void OnPlayerConnected(object sender, PlayerEventArgs e)
        {
            ...
            if (member != null)
            {
                ...
                using (var db = _databaseService.Open<GameContext>())
                {
                    db.ClanMembers.Where(x => x.Id == member.Id).ExecuteUpdate(x => x.SetProperty(e => e.LastLoginDate, member.LastLogin.ToUnixTimeSeconds()));
                }
    ...
    public class Clan : IReadOnlyCollection<ClanMember>
    {
        ...
        public async Task<ClubCommandResult> Approve(Player moderator, ulong accountId)
        {
            ...
            using (var db = _databaseService.Open<GameContext>())
            {
                await db.ClanMembers.Where(x => x.Id == member.Id).ExecuteUpdateAsync(x => x.SetProperty(e => e.State, (byte)member.State));
        ...
        public async Task<ClubCommandResult> Unban(Player moderator, ulong accountId)
        {
            ...
            using (var db = _databaseService.Open<GameContext>())
            {
                ...
                await db.ClanBans.Where(x => x.ClanId == clanId && x.PlayerId == id).ExecuteDeleteAsync();
        ...
        public async Task ChangeAnnouncement(string announcement)
        {
            ...
            {
                var id = (int)Id;
                await db.Clans.Where(x => x.Id == id).ExecuteUpdateAsync(x => x.SetProperty(e => e.Announcement, announcement));
                Announcement = announcement;
            }
        }
        ...
        public async Task ChangeInfo(ClubArea area, ClubActivity activity, string description)
        {
            ...
            {
                var id = (int)Id;
                await db.Clans.Where(x => x.Id == id).ExecuteUpdateAsync(x => x.SetProperty(e => e.Area, (byte)area).SetProperty(e => e.Activity, (byte)activity).SetProperty(e => e.Description, description));
                Area = area;
                Activity = activity;
                Description = description;
            }
            ...
            string question1, string question2, string question3, string question4, string question5)
        {
            ...
            {
                var id = (int)Id;
                await db.Clans.Where(x => x.Id == id).ExecuteUpdateAsync(x => x.SetProperty(e => e.IsPublic, isPublic).SetProperty(e => e.RequiredLevel, requiredLevel).SetProperty(e => e.Question1, question1).SetProperty(e => e.Question2, question2).SetProperty(e => e.Question3, question3).SetProperty(e => e.Question4, question4).SetProperty(e => e.Question5, question5));
                IsPublic = isPublic;
        ...
        public async Task ChangeRole(ClanMember member, ClubRole role)
        {
            using (var db = _databaseService.Open<GameContext>())
            {
                await db.ClanMembers.Where(x => x.Id == member.Id).ExecuteUpdateAsync(x => x.SetProperty(e => e.Role, (byte)role));
                member.Role = role;
            }
        }
